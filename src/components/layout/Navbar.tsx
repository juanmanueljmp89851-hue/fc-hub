...
const navLinks: NavLink[] = [
  { href: "/actualidad", label: "Actualidad" },
  {
    label: "Cartas",
    dropdown: [
      { href: "/jugadores", label: "FC 27" },
      { href: "/jugadores?game=26", label: "FC 26" },
    ],
  },
  { href: "/sbc", label: "SBC" },
...
function NavDropdown({ label, items }: { label: string; items: { href: string; label: string }[] }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="relative flex items-center gap-1 whitespace-nowrap text-xs font-medium text-foreground/70 transition-colors hover:text-accent lg:text-sm"
      >
        {label}
        <svg
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 min-w-[120px] rounded-lg border border-surface-light bg-surface py-1 shadow-xl">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              ...
              <div key={link.label}>
                ...
                {link.dropdown.map((sub) => (
                  <Link
                    ...
                    className="block py-2 pl-4 text-sm font-medium text-foreground/70 transition-colors hover:text-accent"
                  >
                    {sub.label}
                  </Link>
                ))}

...
export function ShareCodeCopy({ shareCode }: { shareCode: string }) {
  const [copied, setCopied] = useState(false);

  const shareUrl = typeof window !== "undefined"
    ? `${window.location.origin}/prode/unirse?code=${shareCode}`
    : `/prode/unirse?code=${shareCode}`;
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-surface-light bg-surface/30 p-3">
      <div className="flex-1">
        ...
        <p className="mt-1 font-mono text-sm text-accent">{shareCode}</p>
      </div>
      <button
        onClick={handleCopy}

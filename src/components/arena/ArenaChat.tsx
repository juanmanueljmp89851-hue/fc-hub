...
import { sendArenaMessage, invokeArenaAdmin } from "@/lib/actions/tournament";
...
export function ArenaChat({ matchId, messages, currentUserId, isPlayer, player1Id, player2Id }: ArenaChatProps) {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setSending(true);
    setError("");
    const result = await sendArenaMessage(matchId, text);
    if (result.error) {
  ...
  async function handleInvoke() {
    ...
    setInvoking(true);
    setError("");
    const result = await invokeArenaAdmin(matchId);
    if (result.error) {
      setError(result.error);
    } else {
      setInvoked(true);
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span className={`text-xs font-bold ${isAdmin ? "text-red-400" : "text-accent"}`}>
                        {msg.user.username}
                        {isAdmin && " (ADMIN)"}
                      </span>
